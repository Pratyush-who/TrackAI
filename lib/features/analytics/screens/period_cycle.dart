...
class _PeriodCyclePageState extends State<PeriodCyclePage> {
        ...
        return Theme(
          data: Theme.of(context).copyWith(
            colorScheme: ColorScheme.fromSeed(
              seedColor: AppColors.primary(isDark),
  ...
  Future<String> _getEnhancedInsights(
    ...
    // Calculate additional metrics from the actual data
    ...
    // Generate more detailed insights based on actual data
    ...
    if (mostCommonSymptoms.isNotEmpty) {
      ...
      // Specific advice based on top symptom
    ...
    // Phase-specific advice based on current data
  ...
  Widget _buildUserDataChart(List<Map<String, dynamic>> cycles, bool isDark) {
    // Create data points based on actual user cycles
    ...
    for (int i = 0; i < cycles.length && i < 30; i++) {
      ...
      // Create intensity based on symptoms
      final symptoms = List<String>.from(cycle['symptoms'] ?? []);
      double intensity = 1.0; // Base intensity
      
      // Increase intensity based on number and severity of symptoms
        ...
        LineChartData(
          ...
          lineBarsData: [
            LineChartBarData(
              ...
              gradient: LinearGradient(
                colors: [
                  AppColors.primary(isDark).withOpacity(0.8),
                  AppColors.primary(isDark),
                ],
              ),
              barWidth: 3,
