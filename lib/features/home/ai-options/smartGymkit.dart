...
Future<void> _loadSavedBulkingResults() async {
    ...
    if (savedResults != null) {
      setState(() {
        ...
        if (userInput != null) {
          ...
          _weightController.text = userInput['weight']?.toString() ?? _weightController.text;
          _heightController.text = userInput['height']?.toString() ?? _heightController.text;
          _ageController.text = userInput['age']?.toString() ?? _ageController.text;
          _selectedActivityLevel = userInput['activityLevel'] ?? _selectedActivityLevel;
          _targetGainController.text = userInput['targetGain']?.toString() ?? _targetGainController.text;
          _timeframeController.text = userInput['timeframe']?.toString() ?? _timeframeController.text;
  ...
  if (weight == null || height == null || age == null || targetGain == null || timeframe == null) {
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
  ...
  if (weight <= 0 || weight > 500) {
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text('Please enter a valid weight (1-500 ${_selectedWeightUnit})'),
        backgroundColor: AppColors.errorColor,
      ),
    );
    return;
  ...
  if (height <= 0 || height > 300) {
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text('Please enter a valid height (1-300 ${_selectedHeightUnit})'),
        backgroundColor: AppColors.errorColor,
      ),
    );
    return;
  ...
  if (targetGain <= 0 || targetGain > 50) {
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text('Please enter a realistic target gain (0.1-50 ${_selectedTargetUnit})'),
        backgroundColor: AppColors.errorColor,
      ),
    );
    return;
    ...
    builder: (BuildContext context) {
      return AlertDialog(
        ...
        content: Column(
          ...
          children: [
            Text('Your target gain rate is ${weeklyGainRate.toStringAsFixed(2)} kg/week.'),
            SizedBox(height: 12),
            Text(
  ...
  // Add this enhanced Bulking Macros Tab to your existing Smartgymkit class
// Replace your existing _buildBulkingMacrosTab method with this:
              ...
              children: [
                ...
                Row(
                  children: [
                    Expanded(
                      child: OutlinedButton.icon(
                        onPressed: () => _showBulkingResultsDialog(_bulkingResults!),
                    ...
                    Expanded(
                      child: ElevatedButton.icon(
                        onPressed: () => _downloadBulkingPlan(_bulkingResults!),
          ...
          Expanded(
            ...
            child: DropdownButtonFormField<String>(
              ...
              items: [
                ...
                DropdownMenuItem(
                  value: 'ft',
                  child: Text('ft/in', style: TextStyle(fontSize: 14)),
                ),
              ],
              onChanged: (value) {
                setState(() {
