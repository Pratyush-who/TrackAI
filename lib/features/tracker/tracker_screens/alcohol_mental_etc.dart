...
class _AlcoholTrackerScreenState extends State<AlcoholTrackerScreen> {
      ...
      builder: (context, themeProvider, child) {
        ...
        return Scaffold(
          ...
          appBar: AppBar(
            ...
            title: Text('Log Alcohol Tracker', style: TextStyle(color: AppColors.textPrimary(isDark), fontSize: 20, fontWeight: FontWeight.bold)),
            ...
            child: SingleChildScrollView(
              ...
              child: Form(
                ...
                child: Column(
                  ...
                  children: [
                    Text('Enter the details for your alcohol tracker entry.', style: TextStyle(color: AppColors.textSecondary(isDark), fontSize: 16)),
  ...
  Widget _buildCravingScale(bool isDark) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text('Craving (1-5)', style: TextStyle(color: AppColors.textPrimary(isDark), fontSize: 16, fontWeight: FontWeight.w600)),
        ...
        Container(
          ...
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text('Rate your craving level', style: TextStyle(color: AppColors.textSecondary(isDark), fontSize: 14)),
              const SizedBox(height: 16),
              Row(
                  ...
                  return GestureDetector(
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(
                        ...
                        border: Border.all(
                          color: isSelected ? AppColors.primary(isDark) : AppColors.textSecondary(isDark),
                          width: 2,
                        ),
                      ),
                      child: Center(
              ...
              Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  Text('Low', style: TextStyle(color: AppColors.textSecondary(isDark), fontSize: 12)),
                  Text('High', style: TextStyle(color: AppColors.textSecondary(isDark), fontSize: 12)),
                ],
              ),
            ],
          ),
  ...
  Widget _buildTextField({required TextEditingController controller, required String label, required String hint, required bool isDark, TextInputType? keyboardType, String? Function(String?)? validator, int maxLines = 1}) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
          ...
          return Container(
            ...
            child: Column(
              children: [
                Row(
                  children: [
                    Expanded(child: _buildTextField(controller: field['key']!, label: 'Field Name', hint: 'e.g., Location', isDark: isDark)),
                    IconButton(
                      onPressed: () {
                        setState(() {
                          field['key']?.dispose();
                ...
                const SizedBox(height: 8),
                _buildTextField(controller: field['value']!, label: 'Field Value', hint: 'e.g., Home', isDark: isDark),
              ],
            ),
          );
        }),
...
class _StudyTrackerScreenState extends State<StudyTrackerScreen> {
      ...
      builder: (context, themeProvider, child) {
        ...
        return Scaffold(
          ...
          appBar: AppBar(
            ...
            title: Text('Log Study Time Tracker', style: TextStyle(color: AppColors.textPrimary(isDark), fontSize: 20, fontWeight: FontWeight.bold)),
            flexibleSpace: Container(decoration: BoxDecoration(gradient: AppColors.backgroundLinearGradient(isDark))),
          ),
          body: Container(
            decoration: BoxDecoration(gradient: AppColors.backgroundLinearGradient(isDark)),
            child: SingleChildScrollView(
              ...
              child: Form(
                ...
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text('Enter the details for your study time tracker entry.', style: TextStyle(color: AppColors.textSecondary(isDark), fontSize: 16)),
                    const SizedBox(height: 24),
  ...
  Widget _buildFocusScale(bool isDark) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text('Focus Level (1-10)', style: TextStyle(color: AppColors.textPrimary(isDark), fontSize: 16, fontWeight: FontWeight.w600)),
        const SizedBox(height: 8),
        Container(
          padding: const EdgeInsets.all(16),
          decoration: BoxDecoration(
          ...
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text('Rate your focus', style: TextStyle(color: AppColors.textSecondary(isDark), fontSize: 14)),
              const SizedBox(height: 16),
              Slider(
  ...
  Widget _buildTextField({required TextEditingController controller, required String label, required String hint, required bool isDark, TextInputType? keyboardType, String? Function(String?)? validator}) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(label, style: TextStyle(color: AppColors.textPrimary(isDark), fontSize: 16, fontWeight: FontWeight.w600)),
          ...
          return Container(
            ...
            child: Column(
              children: [
                Row(
                  children: [
                    Expanded(child: _buildTextField(controller: field['key']!, label: 'Field Name', hint: 'e.g., Resources', isDark: isDark)),
                    IconButton(
                      onPressed: () {
                        setState(() {
                          field['key']?.dispose();
                ...
                const SizedBox(height: 8),
                _buildTextField(controller: field['value']!, label: 'Field Value', hint: 'e.g., Textbook, Online course', isDark: isDark),
              ],
            ),
          );
        }),
...
// Placeholder screens for the remaining trackers
class MentalWellbeingTrackerScreen extends StatelessWidget {
  const MentalWellbeingTrackerScreen({Key? key}) : super(key: key);
  @override Widget build(BuildContext context) => Container(child: Center(child: Text('Mental Wellbeing Tracker - Coming Soon')));
}

class WorkoutTrackerScreen extends StatelessWidget {
  const WorkoutTrackerScreen({Key? key}) : super(key: key);
  @override Widget build(BuildContext context) => Container(child: Center(child: Text('Workout Tracker - Coming Soon')));
}

class WeightTrackerScreen extends StatelessWidget {
  const WeightTrackerScreen({Key? key}) : super(key: key);
  @override Widget build(BuildContext context) => Container(child: Center(child: Text('Weight Tracker - Coming Soon')));
}